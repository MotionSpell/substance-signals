...
class GPACMuxMP4MSS : public GPACMuxMP4 {
	...
	private:
		void declareStreamVideo(const MetadataPktLibavVideo* metadata) final;
		void declareStreamAudio(const MetadataPktLibavAudio* metadata) final;
		void declareStreamSubtitle(const MetadataPktLibavSubtitle* metadata) final;
