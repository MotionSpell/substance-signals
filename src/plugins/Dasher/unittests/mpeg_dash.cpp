...
unittest("dasher: timing computation") {
	...
	struct OutStub : ModuleS {
		OutStub(std::function<void(const char*)> onUpdate_) : onUpdate(onUpdate_){}
		void processOne(Data data) override { onUpdate(std::string((char*)data->data().ptr, data->data().len).c_str()); }
		std::function<void(const char *)> const onUpdate;
	};

	bool received = false;
	auto analyzeMpdTimings = [&received](const char *mpd) {
		if (!received) {
			received = true;
			...
			ASSERT_EQUALS(expectedMpd, std::string(mpd));
		}
	};

	auto mpdAnalyzer = createModule<OutStub>(analyzeMpdTimings);
