...
void declarePipeline(Pipeline &pipeline, const mp42tsXOptions &opt) {
	auto createSink = [&](bool isHLS)->IPipelinedModule* {
		if (isHLS) {
			...
			return pipeline.addModule<Stream::Apple_HLS>(&NullHost, "", "mp42tsx.m3u8", isLive ? Modules::Stream::Apple_HLS::Live : Modules::Stream::Apple_HLS::Static, segmentDurationInMs);
		} else {
			return pipeline.addModule<Out::File>(&NullHost, opt.output);
		}
	};
	...
	auto m2tsmux = pipeline.addModule<Mux::GPACMuxMPEG2TS>(&NullHost, GF_FALSE, 0);
