...
function srt_build {
  lazy_download "srt.tar.gz" "https://github.com/Haivision/srt/archive/v1.5.3.tar.gz"
  ...
  case $host in
    *linux*)
      if ! diff -q "srt/bin/$host/libsrt.so.1.5.3" "$PREFIX/lib/libsrt.so.1.5.3" ; then
        echo "WTF!"
      fi

      cp "srt/bin/$host/libsrt.so.1.5.3" $PREFIX/lib
      ;;
  esac
}
